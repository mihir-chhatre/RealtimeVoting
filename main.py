...
def insert_voters(conn, cur, voter):
    try:
        ...
        print(f'Duplicate voter_id {voter["voter_id"]}, generating new ID.')
        conn.rollback()
        # Generate a new unique voter_id
        new_voter_id = generate_unique_id()
        voter["voter_id"] = new_voter_id
        insert_voters(conn, cur, voter)  # Try inserting again with the new ID
...
if __name__ == "__main__":
    ...
    for i in range(1000):
        ...
        # Check if voter_data is a dictionary
        if isinstance(voter_data, dict):
            ...
            print('Failed to fetch voter data: {}'.format(voter_data))
